...
case class Fork(
  ...
  val massX: Float = if mass == 0 then centerX else (nw.mass * nw.massX + ne.mass * ne.massX + se.mass * se.massX + sw.mass * sw.massX) * 1.0f / mass
  val massY: Float = if mass == 0 then centerY else (nw.mass * nw.massY + ne.mass * ne.massY + se.mass * se.massY + sw.mass * sw.massY) * 1.0f / mass
...
case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: coll.Seq[Body]) extends Quad:
  ...
  def insert(b: Body): Quad =
    if size <= minimumSize then Leaf(centerX, centerY, size, bodies :+ b)
    else
      ...
      var newFork = Fork(nw, ne, sw, se)
      bodies.foreach(bod => newFork = newFork.insert(bod))
      newFork.insert(b)
  ...
  def updated(quad: Quad): Body =
    ...
    def traverse(quad: Quad): Unit = (quad: Quad) match
      ...
      case Fork(nw, ne, sw, se) =>
        if quad.size / distance(x, y, quad.massX, quad.massY) < theta then addForce(quad.mass, quad.massX, quad.massY)
        else {
          traverse(nw)
          traverse(ne)
          traverse(sw)
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface:
  ...
  def +=(b: Body): SectorMatrix =
    ...
    matrix(rowX * sectorPrecision + colX) += b
    this
  ...
  def combine(that: SectorMatrix): SectorMatrix =
    ...
    val tasks = (0 until matrix.length).par.map(i => task(res.matrix(i) = matrix(i).combine(that.matrix(i))))
    val buffers = tasks.map(t => t.join)
    res
