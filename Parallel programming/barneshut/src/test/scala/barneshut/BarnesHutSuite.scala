...
import barneshut.BarnesHut.{model, simulator}
...
class BarnesHutSuite extends munit.FunSuite:
  ...
  test("mergeBoundaries returns correct boundaries") {
    val boundaries = Boundaries()
    boundaries.minX = 1
    boundaries.minY = 1
    boundaries.maxX = 97
    ...
    val boundaries2 = Boundaries()
    boundaries2.minX = 5
    boundaries2.minY = 0
    boundaries2.maxX = 98
    boundaries2.maxY = 96

    val correct = Boundaries()
    correct.minX = 1
    correct.minY = 0
    correct.maxX = 98
    correct.maxY = 97

    new Simulator(null, null):
      val res = mergeBoundaries(boundaries, boundaries2)
      assertEquals(res.minX, correct.minX)
      assertEquals(res.minY, correct.minY)
      assertEquals(res.maxX, correct.maxX)
      assertEquals(res.maxY, correct.maxY)
  }

  test("updateBoundaries updates boundaries correctly") {
    val boundaries = Boundaries()
    boundaries.minX = 1
    boundaries.minY = 1
    boundaries.maxX = 97
    ...
    new Simulator(null, null):
      val res = updateBoundaries(boundaries, body)
      assertEquals(res.minX, correct.minX)
      assertEquals(res.minY, correct.minY)
      assertEquals(res.maxX, correct.maxX)
      assertEquals(res.maxY, correct.maxY)
