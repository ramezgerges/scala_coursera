...
object ParallelCountChange extends ParallelCountChangeInterface:
  ...
  def countChange(money: Int, coins: List[Int]): Int =
    if money == 0 then 1
      ...
      val (left, right) = (countChange(money - coins.head, coins), countChange(money, coins.tail))
      left + right
  ...
  def moneyThreshold(startingMoney: Int): Threshold = (money, coins) => money * 3 < startingMoney * 2

  /** Threshold heuristic based on the total number of initial coins. */
  def totalCoinsThreshold(totalCoins: Int): Threshold = (money, coins) => coins.length * 3 < totalCoins * 2
