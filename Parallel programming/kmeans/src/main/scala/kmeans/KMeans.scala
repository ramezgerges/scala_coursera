...
class KMeans extends KMeansInterface :
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] =
    var parMap = points.groupBy(p => findClosest(p, means))
    ParMap.newCombiner.addAll(means.map(p => if parMap.contains(p) then (p, parMap(p)) else (p, ParSeq.empty))).result()
  ...
  def converged(eta: Double, oldMeans: ParSeq[Point], newMeans: ParSeq[Point]): Boolean =
    if oldMeans.isEmpty then true
      ...
      val r = (0 until oldMeans.length).par
        .map(i => {
          val n = newMeans(i)
          val o = oldMeans(i)
          Math.pow(n.x - o.x, 2) + Math.pow(n.y - o.y, 2) + Math.pow(n.z - o.z, 2)
        })
      ...
      (0 until oldMeans.length).par
        .map(i => {
          val n = newMeans(i)
          val o = oldMeans(i)
          Math.pow(n.x - o.x, 2) + Math.pow(n.y - o.y, 2) + Math.pow(n.z - o.z, 2) <= eta
        })
        .forall(_ == true)
