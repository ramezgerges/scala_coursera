...
object TimeUsage extends TimeUsageInterface {
  ...
  def row(line: List[String]): Row = {
    val ints: Seq[Any] = line.tail.map(str => str.toDouble)
    Row.fromSeq(line.head +: ints)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val classes = columnNames.groupBy(classify)
    val t0 = classes(0).map(new Column(_))
    val t1 = classes(1).map(new Column(_))
    val t2 = classes(2).map(new Column(_))
    (t0, t1, t2)
  }

  def classify(c: String): Int = {
    val sub = c.substring(1, 3)
    if (List("01", "03", "11").contains(sub) || c.startsWith("t1801") || c.startsWith("t1803")) 0
    else if ("05" == sub || c.startsWith("t1805")) 1
    else if (List("02", "04", "06", "07", "08", "09", "10", "11", "12", "13", "14", "15", "16", "17", "18").contains(sub)) 2
    else 3
  }
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column = primaryNeedsColumns.reduce((c1, c2) => c1 + c2).divide(60).alias("primaryNeeds")
    val workProjection: Column = workColumns.reduce((c1, c2) => c1 + c2).divide(60).alias("work")
    val otherProjection: Column = otherColumns.reduce((c1, c2) => c1 + c2).divide(60).alias("other")
    df
