...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    val neighbors = b.legalNeighbors.to(LazyList)
    for
      (b, m) <- neighbors
    yield (b, m :: history)
//    neighbours.to(LazyList).map(neighbour => (neighbour._1, neighbour._2 :: history))
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    for
      (b, h) <- neighbors if !explored.contains(b)
    yield (b, h)
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if initial.isEmpty then LazyList()
      ...
      val fromFirst = newNeighborsOnly(neighborsWithHistory(first._1, first._2), explored)
      fromFirst ++ from(initial.tail ++ fromFirst, explored ++ fromFirst.map(p => p._1))
//      val next =
//        for
...
//      next #::: from(next, explored)
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] =
    for
      (block, listOfMoves) <- pathsFromStart if block == Block(goal, goal)
  ...
  lazy val solution: List[Move] =
    if pathsToGoal.isDefinedAt(0) then pathsToGoal(0)._2.reverse
    else Nil
