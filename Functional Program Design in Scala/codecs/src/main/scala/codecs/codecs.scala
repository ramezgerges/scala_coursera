...
trait DecoderInstances:
  ...
  /**
    ...
    */
  given arrDecoder[A] (using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction(jsonArr => jsonArr match
      case Json.Arr(l) => l match
        case Nil => Some(Nil)
        case jsonElem :: tail =>
          val decodedElem = decoder.decode(jsonElem)
          val decodedTail = arrDecoder.decode(Json.Arr(tail))
          if decodedElem.isDefined && decodedTail.isDefined then Some(decodedElem.get :: decodedTail.get) else None
      case _ => None
    )
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction(objJson => objJson match
      case Json.Obj(fields) => if fields.contains(name) then decoder.decode(fields(name)) else None
      case _ => None
    )
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    Decoder.fromFunction(jsonObj => jsonObj match
      case Json.Obj(_) =>
        val name = Decoder.field[String]("name").decode(jsonObj)
        val age = Decoder.field[Int]("age").decode(jsonObj)
        if name.isDefined && age.isDefined then Some(Person(name.get, age.get)) else None
      case _ => None
    )
...
trait ContactsCodecs:
  ...
  given Decoder[Contacts] =
      Decoder.field[List[Person]]("people").transform(list => Contacts(list))
