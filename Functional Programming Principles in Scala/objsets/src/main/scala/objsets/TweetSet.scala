...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet = throw new NoSuchElementException("Empty set doesn't have a most retweeted tweet")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    if (p(elem)) then left.filterAcc(p, acc.union(right.filter(p))).incl(elem)
    else left.filterAcc(p, acc.union(right.filter(p)))
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet =
    val m = if acc.retweets > elem.retweets then acc else elem
    val l = left.mostRetweetedAcc(m)
    val r = right.mostRetweetedAcc(m)
    if r.retweets > l.retweets then r else l
...
object GoogleVsApple:
  ...
  def f(l: List[String]): TweetSet =
    if l.isEmpty then Empty()
    else allTweets.filter(t => t.text.contains(l.head)).union(f(l.tail))
